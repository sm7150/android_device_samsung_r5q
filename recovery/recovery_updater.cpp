...
#define VENDOR_SPL_STR "ro.vendor.build.security_patch="
#define VENDOR_SPL_STR_LEN 31
#define VENDOR_SPL_BUF_LEN 11
...
static int get_spl_date(char* ver_str, size_t len) {
    int ret = 0;
    int fd;
    off64_t spl_size;
    char* spl_data = NULL;
    ...
    fd = open(VENDOR_PART_PATH, O_RDONLY);
    if (fd < 0) {
        ret = errno;
        goto err_ret;
    }

    spl_size = lseek64(fd, 0, SEEK_END);
    if (spl_size == -1) {
        ret = errno;
        goto err_fd_close;
    }

    spl_data = (char*)mmap(NULL, spl_size, PROT_READ, MAP_PRIVATE, fd, 0);
    if (spl_data == (char*)-1) {
        ret = errno;
        goto err_fd_close;
    }

    /* Do Boyer-Moore search across vendor data */
    offset = bm_search(spl_data, spl_size, VENDOR_SPL_STR, VENDOR_SPL_STR_LEN);
    if (offset != NULL) {
        snprintf(ver_str, len, "%s", offset + VENDOR_SPL_STR_LEN);
    } else {
        ret = -ENOENT;
    }

    munmap(spl_data, spl_size);
err_fd_close:
    close(fd);
err_ret:
    return ret;
...
/* verify_vendor("VENDOR_SPL", "VENDOR_SPL", ...) */
Value* VerifySplFn(const char* name, State* state,
                     const std::vector<std::unique_ptr<Expr>>& argv) {
    char current_spl_date[VENDOR_SPL_BUF_LEN];
    int ret;
    struct tm tm1, tm2;

    ret = get_spl_date(current_spl_date, VENDOR_SPL_BUF_LEN);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
                          "%s() failed to read current vendor build SPL: %d", name, ret);
    }
    ...
    strptime(current_spl_date, "%Y-%m-%d", &tm1);

    for (auto& spl_date : args) {
        memset(&tm2, 0, sizeof(tm));
        strptime(spl_date.c_str(), "%Y-%m-%d", &tm2);
...
void Register_librecovery_updater_r5q() {
    RegisterFunction("r5q.verify_vspl", VerifySplFn);
}
